layui.config({
        ...
        $api.load_ys_ProductTask(req,function (res) {
            ...
            for(var i = 0 ;i<list.length;i++){
               detail += '<div class="father">' +
                   ...
                   '               <div class="layui-input-block">\n' +
                   '                   <select name="name" type="" class="Personincharge">\n' +
    ...
    function getPerson() {
    $api.Get_ys_lisSysUsers(null,null,function (res) {
        var data = res.data;
        if(data.length > 0){
            var html = '';
            for(var i=0;i<data.length;i++){
                html += '<option value="'+data[i].id+'">'+data[i].name+'+'+data[i].departname+'</option>>';
            }
    ...
    form.on("submit(editProductTask)", function (data) {
        ...
        for(var i=0;i<zujian.length;i++){
                ...
                // alert(DetailName);
                var DetailNumber =  $($($($(zujian[i]).children('div')[2]).children('.layui-input-block')[0]).children('input')[0]).val();
                // alert(DetailNumber);
...
function zAppend(a){
    ...
    $(a.prev()).append('<div class="father">' +
        ...
        '               </div>\n' +
        '           </div>\n' +
        ' <div class="layui-inline layui-form-item 1">\n' +
        '               <label class="layui-form-label">负责人</label>\n' +
        '               <div class="layui-input-block">\n' +
        '                   <select name="name" type="" class="Personincharge">\n' +
        '                       <option value="">--请选择--</option>\n' +
