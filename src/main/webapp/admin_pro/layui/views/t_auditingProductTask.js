layui.config({
     ...
     * 表单提交
     * */
    form.on("submit(auditingProductTask)", function (data) {
        ...
        $api.Update_ys_ProductTaskAuditing(req,"",function (data) {
            layer.msg("已通过审核！",{time:1000},function () {
    ...
    form.on("submit(rejectProductTask)", function (data) {
        ...
        // alert(JSON.stringify(req));
        $api.Update_ys_ProductTaskReject(req,"",function (data) {
            layer.msg("审核已被驳回！",{time:1000},function () {
                layer.closeAll("iframe");
                //刷新父页面
                parent.location.reload();
            });
